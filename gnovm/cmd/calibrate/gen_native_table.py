...
def emit_go_table(rows, out):
    ...
    out.write("// NOT a paste-replace for gnovm/stdlibs/native_gas.go — that file\n")
    out.write("// uses values from a clean-environment reference bench run that may\n")
    out.write("// differ from this regenerated output (bench-to-bench wall-clock noise\n")
    out.write("// on dev hardware). Always re-bench in a clean environment, compare\n")
    out.write("// to production, and reconcile before pasting. See the project's\n")
    out.write("// CLAUDE.md verification rules for the consensus-relevant flow.\n")
