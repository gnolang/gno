...
func TestLintApp(t *testing.T) {
	tc := []testMainCase{
		...
		{
			args:                []string{"lint", "../../tests/integ/run_main/"},
			stderrShouldContain: "./../../tests/integ/run_main: parsing gno.mod at ./../../tests/integ/run_main: gno.mod file not found in current or any parent directory (code=1)\n",
			errShouldBe:         "exit code: 1",
		},
		{
			args: []string{"lint", "../../tests/integ/several_lint_errors/main.gno"},
			stderrShouldContain: strings.Join([]string{
				"../../tests/integ/several_lint_errors/main.gno:5:5: expected ';', found example (code=2)",
				"../../tests/integ/several_lint_errors/main.gno:6:2: expected '}', found 'EOF' (code=2)",
			}, "\n") + "\n",
			errShouldBe: "exit code: 1",
		},
		{
			...
			stderrShouldContain: func() string {
				lines := []string{
					"../../tests/integ/several_files_multiple_errors/file2.gno:3:5: expected 'IDENT', found '{' (code=2)",
