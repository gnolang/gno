...
func TestLintApp(t *testing.T) {
	tc := []testMainCase{
		...
		},
		{
			...
			simulateExternalRepo: true,
			stderrShouldBe:       "gno.land/r/test/render_invalid1/main.gno:5: the 'Render' function signature is incorrect for the 'main' package. the signature must be of the form: func Render(string) string (code=gnoParserError)\n",
			errShouldBe:          "exit code: 1",
		},
		{
			args:                 []string{"lint", "."},
			...
			simulateExternalRepo: true,
			stderrShouldBe:       "gno.land/r/test/render_invalid2/main.gno:5: the 'Render' function signature is incorrect for the 'main' package. the signature must be of the form: func Render(string) string (code=gnoParserError)\n",
			errShouldBe:          "exit code: 1",
		},
		{
			args:                 []string{"lint", "."},
			...
			simulateExternalRepo: true,
			stderrShouldBe:       "gno.land/r/test/render_invalid3/main.gno:5: the 'Render' function signature is incorrect for the 'main' package. the signature must be of the form: func Render(string) string (code=gnoParserError)\n",
			errShouldBe:          "exit code: 1",
		},
		{
			args:                 []string{"lint", "."},
