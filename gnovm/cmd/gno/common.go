...
func reportError(reporter lint.Reporter, dir, pkgPath string, err error) {
	...
	case *gno.PreprocessError:
		err2 := err.Unwrap()
		code, msg, loc := guessIssueFromError(dir, pkgPath, err2, gnoPreprocessError)
		reportIssue(reporter, code, msg, loc)
	...
	default:
		...
		if len(errors) == 1 {
			code, msg, loc := guessIssueFromError(dir, pkgPath, err, gnoUnknownError)
			reportIssue(reporter, code, msg, loc)
			return
		}
...
func guessIssueFromError(dir, pkgPath string, err error, code gnoCode) (gnoCode, string, string) {
	parsedError := strings.TrimSpace(err.Error())
	match := gno.ReErrorLine.Match(parsedError)
	if match == nil {
		loc := fmt.Sprintf("%s:0", filepath.Clean(pkgPath))
		return code, err.Error(), loc
	}
	...
	return code, strings.TrimSpace(errMsg), errPath + ":" + errLoc
}
