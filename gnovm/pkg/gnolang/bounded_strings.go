...
// (when Machine.BoundedPanicRender is set), and are a separate
// renderer from the Sprint / String methods used by filetests, REPL,
// gno run, debug output, and other trusted-context consumers. Those
// methods have their own size cap (see boundedBuilder in
// values_string.go) but invoke user .String()/.Error() and so must
// not be reused here.
//
// DESIGN INVARIANTS:
//
