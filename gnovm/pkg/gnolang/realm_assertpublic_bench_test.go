...
// clearTypePrivacyMemo drops both the committed cache and this store's
// per-tx buffer, so the next typeHasPrivateDep is a guaranteed miss.
func clearTypePrivacyMemo(store Store) {
	ds := asDefaultStore(store)
	ds.typePrivacyCache.m = make(map[TypeID]bool)
	ds.pendingTypePrivacy = nil
}

// warmTypePrivacyMemo runs one walk and promotes its buffered verdict into
// the committed cache (as a real commit would), leaving the buffer empty
// so subsequent lookups hit the shared cache.
func warmTypePrivacyMemo(store Store, rlm *Realm, t Type) {
	rlm.assertTypeIsPublic(store, t, map[TypeID]struct{}{})
	ds := asDefaultStore(store)
	for tid, v := range ds.pendingTypePrivacy {
		ds.typePrivacyCache.set(tid, v)
	}
	ds.pendingTypePrivacy = nil
}
...
// type-graph walk. The memo is cleared each iteration so every call is a
// miss.
func BenchmarkAssertTypeIsPublic_ColdAcyclic(b *testing.B) {
	...
	for b.Loop() {
		clearTypePrivacyMemo(store) // force a cold memo each iteration
...
// commit after the first: a TypeID already in the committed cache — a hit.
...
func BenchmarkAssertTypeIsPublic_WarmAcyclic(b *testing.B) {
	...
	rlm := NewRealm(rootPath)
	root := buildPublicType(pkgPaths, rootPath, 20)
	warmTypePrivacyMemo(store, rlm, root) // commit the verdict once
...
// hit the committed cache across commits like any other.
func BenchmarkAssertTypeIsPublic_WarmSelfReferential(b *testing.B) {
	...
	rlm := NewRealm(pkgPath)
	node := buildSelfReferentialType(pkgPath)
	warmTypePrivacyMemo(store, rlm, node) // commit the verdict once
