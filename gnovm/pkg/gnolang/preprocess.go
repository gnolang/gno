...
func findStructDeclDependentNames(store Store, last BlockNode, n Node, dst map[Name]struct{}, pkgPath string) {
	switch cn := n.(type) {
	case *constTypeExpr:
	...
	case *TypeDecl:
		findStructDeclDependentNames(store, last, cn.Type, dst, pkgPath)
	default:
		panic(fmt.Sprintf(
			"unexpected node: %v (%v)",
			n, reflect.TypeOf(n)))
