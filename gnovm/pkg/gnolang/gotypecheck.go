...
const (
	...
	TCGLatestAddpkg                      // require latest gno.mod gno version, but forbid import draft packages too
...
func (m TypeCheckMode) RequiresLatestGnoMod() bool {
	return m == TCLatestStrict || m == TCGLatestAddpkg
}
...
func (gimp *gnoImporter) ImportFrom(pkgPath, _ string, _ types.ImportMode) (*types.Package, error) {
	...
	if gimp.tcmode == TCGLatestAddpkg && mod != nil && mod.Draft {
