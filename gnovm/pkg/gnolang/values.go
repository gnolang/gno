...
func (tv *TypedValue) ComputeMapKey(m *Machine, store Store, omitType bool) (key MapKey, isNaN bool) {
	return tv.computeMapKey(m, store, omitType, 0)
}

func (tv *TypedValue) computeMapKey(m *Machine, store Store, omitType bool, depth int) (key MapKey, isNaN bool) {
	...
	case *ArrayType:
		...
		if av.Data == nil {
			...
			for i := range al {
				...
				mk, isNaN := ev.computeMapKey(m, store, omitTypes, depth+1)
				if isNaN {
					return "", true
				}
	...
	case *StructType:
		...
		for i := range sl {
			...
			mk, isNaN := fv.computeMapKey(m, store, omitTypes, depth+1)
			if isNaN {
				return "", true
			}
			bz = binary.AppendUvarint(bz, uint64(len(mk)))
