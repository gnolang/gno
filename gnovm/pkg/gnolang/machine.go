...
//
// Batching is still valuable even with concrete GasMeter: it avoids
// dereferencing the GasMeter pointer (separate cache line) on every op.
// The GasMeter nil check is always true but kept as an inlining hint
// (the Go inliner discounts nil-guarded blocks).
...
const (
	// CPU gas costs recalibrated on Xeon 8168, cpuBaseNs=5.2.
	// Generated by: make calibrate.gas (gen_analysis.py)
	...
	OpCPUPanic2              = 9
	OpCPUReturn              = 27
	OpCPUReturnAfterCopy     = 34
	OpCPUReturnFromBlock     = 32
	OpCPUReturnToBlock       = 22
	...
	OpCPULor   = 16
	OpCPULand  = 16
	OpCPUEql   = 18 // max(int=17, float64=18); parameterized cases added in handler
	OpCPUNeq   = 16
	OpCPULss   = 14
	OpCPULeq   = 14
	OpCPUGtr   = 14
	OpCPUGeq   = 14
	OpCPUAdd   = 34 // max(int=17, float64=31, string=34)
	OpCPUSub   = 29 // max(int=16, float64=29)
	OpCPUBor   = 15
	OpCPUXor   = 15
	OpCPUMul   = 29 // max(int=16, float64=29)
	OpCPUQuo   = 34 // max(int=16, float64=34)
	OpCPURem   = 16
	OpCPUShl   = 16
	OpCPUShr   = 16
	OpCPUBand  = 15
	...
	OpCPUMapLit       = 103 // base; per-entry added in handler
	OpCPUStructLit    = 43  // base; per-field added in handler
	OpCPUFuncLit      = 34  // base; per-capture added in handler
	OpCPUConvert      = 58  // max(int->int64=31, str->bytes=58); parameterized cases in handler
	...
	OpCPUInc         = 18
	OpCPUDec         = 40 // max(int=18, float64=40)
