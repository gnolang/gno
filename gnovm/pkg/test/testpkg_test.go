		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, pkg := range subPkgs {
				...
				if errors.Is(err, fs.ErrNotExist) {
					t.Errorf("Autogenerated %q/gnomod.toml missing. Generate, then rerun test.", pkg.Dir)
				} else if err != nil {
					t.Errorf("This unexpected error would be ignored: %q", err.Error())
				}

				// Determine pkgPath from gno.mod.
				gnoPkgPath := mod.Module
