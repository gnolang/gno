...
func Load(conf LoadConfig, patterns ...string) (PkgList, error) {
	...
	expanded, err := expandPatterns(conf.GnoRoot, loaderCtx, conf.Out, fsys, patterns...)
	if err != nil {
		return nil, err
	}

	pkgs, err := loadMatches(conf.Out, conf.Fetcher, expanded, fsys, conf.Fset)
	if err != nil {
		return nil, err
	}
	...
	localDeps := discoverPkgsForLocalDeps(conf, fsys, loaderCtx)
	...
	for {
		...
		for _, imp := range pkg.ImportsSpecs.Merge(importKinds...) {
			...
			if gnolang.IsStdlib(imp.PkgPath) {
				...
				markDepForVisit(loadSinglePkg(conf.Out, conf.Fetcher, dir, fsys, conf.Fset))
				continue
			}

			// check if this package is present in current workspace or extra workspace roots
			if dir, ok := localDeps[imp.PkgPath]; ok {
				markDepForVisit(loadSinglePkg(conf.Out, nil, dir, fsys, conf.Fset))
				continue
			}
...
func findLoaderContext(fsys fs.FS, wd string) (*loaderContext, error) {
	dir, err := findWorkspaceRootDir(fsys, wd)
	switch {
	case err == nil:
...
func findWorkspaceRootDir(fsys fs.FS, absPath string) (string, error) {
...
func discoverPkgsForLocalDeps(conf LoadConfig, fsys fs.FS, loaderCtx *loaderContext) map[string]string {
	...
	for _, root := range roots {
		...
		_ = fs.WalkDir(walkFS, ".", func(path string, d fs.DirEntry, err error) error {
			...
			case "gnomod.toml", "gno.mod":
				...
				// find pkg path
				gm, err := gnomod.ParseFSDir(fsys, dir)
				if err != nil {
