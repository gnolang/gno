...
func SetupGnoimport(p *testscript.Params, buildDir string) error {
	...
	// Determine the path to the gnoimport binary within the build directory
	gnoimpBin := filepath.Join(buildDir, "gnoimport")
	if _, err := os.Stat(gnoimpBin); err != nil {
		...
		gnoArgsBuilder := []string{"build", "-o", gnoimpBin}
		...
		gnoArgsBuilder = append(gnoArgsBuilder, filepath.Join(gnoroot, "contribs", "gnoimports"))

		if err = exec.Command("go", gnoArgsBuilder...).Run(); err != nil {
			return fmt.Errorf("unable to build gnoimport binary: %w", err)
		}
	}
	...
	p.Cmds["gnoimport"] = func(ts *testscript.TestScript, neg bool, args []string) {
		err := ts.Exec(gnoimpBin, args...)
		if err != nil {
