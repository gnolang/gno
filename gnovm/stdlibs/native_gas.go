...
var calibratedNativeGas = []nativeGasEntry{
	...
	{Pkg: "chain/runtime", Fn: "PayGas", Base: 600, Slope: 1319, SlopeIdx: -1, SlopeKind: SizeNumCallFrames},                                                   // per-frame: CurrentRealm scans the call stack (slope mirrors getRealm) + balance read (heuristic, recalibrate via gnovm/cmd/calibrate)
	{Pkg: "chain/runtime", Fn: "PayStorage", Base: 600, Slope: 1319, SlopeIdx: -1, SlopeKind: SizeNumCallFrames},                                               // per-frame: CurrentRealm scans the call stack (slope mirrors getRealm) + balance read (heuristic, recalibrate via gnovm/cmd/calibrate)
