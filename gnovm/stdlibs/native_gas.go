...
var calibratedNativeGas = []nativeGasEntry{
	...
	{Pkg: "chain/reflect", Fn: "objectInfo", Base: 1200, SlopeIdx: -1, SlopeKind: SizeFlat},                                                                    // draft (Apple M1), median 1196ns stamped: sha256+bech32 address derivation dominates, plus the creating-realm lookup. Unstamped 106ns, no-identity 90ns; the stamped path sets the price. Flat: the preimage is a fixed 20-byte realm hash plus a uint64, so nothing here scales with caller input. Reference rows on the same run read 52 / 208 / 3268ns against table values of 45 / 148 / 2631 (chain/runtime.ChainID, chain/runtime.getSessionInfo, chain.pubKeyAddress), i.e. this machine reads 0.71-0.87x of the table, so the unscaled median overcharges rather than under. Re-fit on the reference Xeon 8168 with the rest.
