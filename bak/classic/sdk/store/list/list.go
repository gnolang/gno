...
func (m List) Len() (res uint64) {
	...
	amino.MustUnmarshalSized(bz, &res)
	return
}
...
func (m List) Get(index uint64, ptr interface{}) error {
	...
	return amino.UnmarshalSized(bz, ptr)
}
...
func (m List) Set(index uint64, value interface{}) {
	bz := amino.MustMarshalSized(value)
...
func (m List) Push(value interface{}) {
	length := m.Len()
	m.Set(length, value)
	m.store.Set(LengthKey(), amino.MustMarshalSized(length+1))
}
...
func (m List) Iterate(ptr interface{}, fn func(uint64) bool) {
	...
	for ; iter.Valid(); iter.Next() {
		v := iter.Value()
		amino.MustUnmarshalSized(v, ptr)
