...
type DeviceInfo struct {
	...
	// unexported reference for Open()
	usbhidDev *usbhid.Device
}
...
// Open connects to the HID device described by this DeviceInfo.
func (info DeviceInfo) Open() (*Device, error) {
	if info.usbhidDev == nil {
		return nil, errors.New("hid: no device reference")
	}
	if err := info.usbhidDev.Open(false); err != nil {
		return nil, err
	}
	return &Device{
		DeviceInfo: info,
