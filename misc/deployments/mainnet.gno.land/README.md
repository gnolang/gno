...
- **Validators**: 4 founding validators — Gnocore, OnBloc, Samourai Crew, Berty — one each, power 60 (one dark = one quarter lost, safely below the one-third halt boundary). Samourai Crew's keys and Berty's consensus pair are real; Gnocore and OnBloc keys, plus the remaining operators, are `TODO(mainnet)` placeholders pending each org's ceremony.
