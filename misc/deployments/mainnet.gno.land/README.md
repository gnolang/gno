...
> **Status: WORK IN PROGRESS — not launchable.** The genesis time is decided (2026-09-12T13:00:00Z); the build stays refused until the allocation sheet is re-pinned with `-vesting-start` equal to it, and the hardcoded-address audit is still running — grep `TODO(mainnet)` in this folder. `CHECKSUMS_DATA` stays unlocked until every value is final.
