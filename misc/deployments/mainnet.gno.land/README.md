...
- **Validators**: 4 founding validators — Gnocore, OnBloc, Samourai Crew, Berty — one each, power 60 (one dark = one quarter lost, safely below the one-third halt boundary). OnBloc, Samourai Crew and Berty are fully real (consensus pair + operator each); only Gnocore's key and operator remain `TODO(mainnet)` placeholders pending its ceremony.
