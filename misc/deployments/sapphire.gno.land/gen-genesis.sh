#!/usr/bin/env bash
# gen-genesis.sh — sapphire genesis builder (single-file pipeline).
#
# sapphire is a FRESH chain — no hardfork, no historical replay. This
...
#      r/gov/dao/v3/impl. No transfer lock — sapphire is unrestricted.
...
#   6. Balances: the faucet accounts at FAUCET_BALANCE each, plus exact-burn
...
# Initial sapphire validator set. Format: "name power address pub_key".
...
# are NOT in this repo). Transfers are unrestricted on sapphire, so no
# unrestrict step is needed anywhere.
#
# 1e18 leaves ~9.2x headroom under int64 max per account. Keep
# (count × FAUCET_BALANCE) under int64 max (~9.22e18): Coin.Add panics
# on int64 overflow, so consolidating every faucet into one account must
...
#   <sha256>  <path-relative-to-sapphire.gno.land>
#
...
# <path> relative to SAPPHIRE_DIR) in the inline CHECKSUMS_DATA heredoc,
# and one of:
...
verify_checksum() {
  local path="$1"
  if [ -z "${SAPPHIRE_DIR:-}" ]; then
    die "verify_checksum: SAPPHIRE_DIR not set"
  fi
  if [ ! -f "$path" ]; then
    die "verify_checksum: $path does not exist"
  fi

  local rel="${2:-${path#"$SAPPHIRE_DIR"/}}"
  local got
  got=$(sha256_of "$path")
...
See misc/deployments/sapphire.gno.land/README.md for what the genesis
contains.
EOF
}
...
print_substep "1.1" "SAPPHIRE_DIR=$SAPPHIRE_DIR"
...
# transfer lock, no unrestricted-accounts proposals — sapphire transfers
# are unrestricted.
...
print_substep "7.1" "Building CSV from INITIAL_VALSET + INITIAL_VALSET_OPERATORS..."
{
  ...
  for i in "${!INITIAL_VALSET[@]}"; do
    ...
    printf '%s,%s,%s,sapphire founding validator (%s),cloud\n' \
...
# Fee payers (exact-burn, land at zero) + the faucets, one sheet. One
...
printf '\n### sapphire build complete: genesis.json (%s, sha256=%s) ###\n' \
