# Joining pearl as a validator

How to run a full node on **pearl** and put yourself forward as a validator candidate. This assumes you're comfortable with Go, Docker, and `gnokey` — it only covers what's specific to pearl.
...
Everything is built from the **`chain/pearl`** branch (<https://github.com/gnolang/gno/tree/chain/pearl>).
...
```shell
docker build --target gnoland -t gnoland:pearl .
```
...
Download `genesis.json` from the [release page](https://github.com/gnolang/gno/releases/tag/chain%2Fpearl):
...
The registration transaction costs a gas fee, so your operator account needs GNOT. If it's empty, request a drip for your `g1...` address from the pearl faucet at <https://pearl.testnets.gno.land/faucet>.
...
You can review registered valopers and the current set at <https://pearl.testnets.gno.land/r/gnops/valopers> and <https://pearl.testnets.gno.land/r/sys/validators/v3>.
