...
> **Status:** standalone library, maintained best-effort. gnoweb's state
> explorer renders state server-side and no longer consumes this package;
> it is kept here for external tooling (block explorers, IDE plugins,
> JS SDKs) that want to decode Amino JSON in the browser. Verified against
> chain encoding circa the commit that introduced this note — may drift if
> the wire format evolves; the test suite is the contract of record.
