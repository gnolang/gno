...
- In crossing functions (`func F(cur realm, ...)`), the first `cur realm` is guaranteed current by the runtime, so `cur.Previous()` is safe to use directly — a `cur.IsCurrent()` check on the first `cur` is redundant (always true; see `gnovm/tests/files/zrealm_iscurrent.gno`). The check is **required** when a realm value is threaded into a non-crossing helper as a secondary parameter (`func H(_ int, rlm realm, ...)`): call `rlm.IsCurrent()` before trusting `rlm.Previous()`/`rlm.Address()`/`rlm.PkgPath()`, or a caller can pass `cur.Previous()` or a stored realm and forge identity (designation-forgery). Never use `chain/runtime/unsafe.PreviousRealm()` in a crossing function — it bypasses this frame verification. Any import of `chain/runtime/unsafe` alongside `cur realm` parameters is a red flag.
