...
- In a crossing function, derive caller identity from `cur.Previous()` and write no `cur.IsCurrent()` guard: every entry from another realm mints a fresh `cur`, so the check refuses nobody. `IsCurrent()` belongs on a realm parameter that is not in first position, the `rlm` of `Send(_ int, rlm realm, ...)`, where it refuses a stale value.
