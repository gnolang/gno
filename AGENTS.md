...
- In crossing functions (`func F(cur realm, ...)`), derive caller identity from `cur.Previous()` and write no `cur.IsCurrent()` guard, since every entry from another realm mints a fresh `cur` and the check refuses nobody. Never read `chain/runtime/unsafe.PreviousRealm()` or `unsafe.CurrentRealm()` for caller identity in a crossing function, since both walk the frame stack and answer from wherever they are called. The `unsafe.OriginCaller()` and `unsafe.OriginSend()` tx-origin primitives are a separate case: they have no `cur` substitute and stay legitimate for event emission or fee attribution.
