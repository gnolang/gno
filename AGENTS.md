...
- In a crossing function, derive caller identity from `cur.Previous()` and write no `cur.IsCurrent()` guard: the runtime puts `cur` into the current realm-context on entry, so the check is always true. `IsCurrent()` belongs on a realm value a caller hands over as an ordinary argument, which may be left over from an earlier call.
