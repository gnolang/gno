...
- In crossing functions (`func F(cur realm, ...)`), `cur.IsCurrent()` authenticates nothing: every entry from another realm mints a fresh `cur`. The check belongs on a realm value the caller hands over as an ordinary parameter, the `rlm` of a non-crossing `Send(_ int, rlm realm, ...)`. Never use `chain/runtime/unsafe.PreviousRealm()` in a crossing function — it stack-walks, while `cur.Previous()` reads the chain the `cur` value itself carries. Any import of `chain/runtime/unsafe` alongside `cur realm` parameters is a red flag.
