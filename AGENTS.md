...
- In a crossing function, derive caller identity from `cur.Previous()` and write no `cur.IsCurrent()` guard: the runtime mints a fresh `cur` on every entry from another realm, so the check refuses nobody. `IsCurrent()` belongs on a realm parameter other than a crossing function's first, which carries whatever the caller passed and may be left over from an earlier call.
