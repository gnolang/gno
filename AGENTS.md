...
- In a plain crossing function (`func F(cur realm, ...)`), the frame adopts whatever `cur` it is handed, so `cur.IsCurrent()` there is always true and proves nothing; read `cur.Previous()` directly. A crossing **method** (`func (r T) M(cur realm, ...)`) is the opposite: its frame never adopts the value, `IsCurrent()` resolves against an outer crossing frame and can be false, so guard it before any authority read. The check is required for every other realm value a function receives. On a secondary realm parameter of a non-crossing helper (`func H(_ int, rlm realm, ...)`) or of a crossing function, call `rlm.IsCurrent()` before trusting `rlm.Previous()`/`rlm.Address()`/`rlm.PkgPath()`, or a caller can pass `cur.Previous()` or another forwarded realm value and forge identity. Never use `chain/runtime/unsafe.PreviousRealm()` in a crossing function: it ignores the `cur` token the runtime already minted for this, and the same call in a non-crossing helper names a realm above the immediate caller. Any import of `chain/runtime/unsafe` alongside `cur realm` parameters is a red flag.
