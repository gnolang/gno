...
- In a crossing function, derive caller identity from `cur.Previous()` and write no `cur.IsCurrent()` guard. Every entry from another realm mints a fresh `cur`, so the check refuses nobody. `IsCurrent()` belongs on a realm value a caller hands over as an ordinary parameter, `(_ int, rlm realm, ...)`, where a stale token is refused.
- Never read `chain/runtime/unsafe.PreviousRealm()` in a crossing function. It walks the frame stack and answers from wherever it is called; `cur.Previous()` names the realm that was current when `cur` was minted. Delete any import of `chain/runtime/unsafe` sitting beside a `cur realm` parameter.
