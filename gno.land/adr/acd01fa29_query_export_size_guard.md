...
- **Post-marshal size check** (the first fix proposed). `amino.MarshalJSON`
...
- **Thread an `*Allocator` through export and charge every copy** (also in
  that fix). Bounds the export copy, but does nothing for the marshal (the
...
- `gnovm/pkg/gnolang/values_export_limit_test.go` — limiter unit tests: single
  ...
  removed.
- `gno.land/pkg/sdk/vm/keeper_large_input_test.go` — rejection through every wired
...
- `keeper_large_input_test.go` — `TestQueryEval_DepthGuard`: a `Deep(5000)` query on
