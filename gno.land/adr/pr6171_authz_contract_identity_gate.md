...
5. **`SimpleExecutor.Execute` is invocable only from the `r/gov/dao` namespace**
   (`r/gov/dao/types.gno`). This is option 1 from that review, chosen as the
...
- **Only unexport `valopers.Auth()`.** Rejected: it fixes one consumer and
  leaves the package default unsafe for the next one. The hazard is the
  package default, not the getter.
...
- **Behavior change for existing consumers.** A plain `NewContractAuthority`
  ...
  `p/moul/authz/v0/filetests/z_contract_authority_shape_filetest.gno` — see
  Coverage.
...
adapts the review's characterization test into its inverted, regression form
