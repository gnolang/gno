...
To test:
  gno test .`)}function b(){i(`Testing requires a running gno node.

To test locally:
  gno test .`)}function E(){i(`Formatting requires server-side gno fmt (coming soon).

To format locally:
  gno fmt -w `+e[l].name)}function T(){e[l].content=a.value;let n=e.length===1?e[0].content:e.map(c=>`// --- ${c.name} ---
${c.content}`).join(`
...
${r}`)})}function $(){i("// Run code to see output here")}a.addEventListener("keydown",n=>{if(n.ctrlKey&&n.key==="Enter"){n.preventDefault(),v();return}if(n.key==="Tab"&&!n.shiftKey){n.preventDefault();let t=a.selectionStart,o=a.selectionEnd;a.value=`${a.value.substring(0,t)}	${a.value.substring(o)}`,a.selectionStart=a.selectionEnd=t+1}});let k={runCode:v,runTests:b,formatCode:E,shareCode:T,clearOutput:$};d.querySelectorAll("[data-action]").forEach(n=>{let t=n.getAttribute("data-action");if(!t)return;let o=t.match(/^(\w+)->playground#(\w+)$/);if(!o)return;let[,r,c]=o,y=k[c];y&&n.addEventListener(r,y)}),p()}export{L as default};
