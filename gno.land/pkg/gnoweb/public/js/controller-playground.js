...
To format locally:
  gno fmt -w `+this.files[this.activeFile].name)}async shareCode(){this.files[this.activeFile].content=this.codeEl.value;let t=this.files.length===1?this.files[0].content:this.files.map(a=>`// --- ${a.name} ---
${a.content}`).join(`

`),e=new TextEncoder().encode(t),i=new CompressionStream("deflate-raw"),n=i.writable.getWriter();n.write(e),n.close();let s=await new Response(i.readable).arrayBuffer(),o=Array.from(new Uint8Array(s),a=>String.fromCharCode(a)).join(""),l=`${window.location.origin}/_/play?code=${encodeURIComponent(btoa(o))}&z`;if(l.length>u){this._setOutput(`Error: code is too large to share via URL.

Try reducing the code or splitting into a deployed package.`,!0);return}navigator.clipboard.writeText(l).then(()=>this._setOutput("Share URL copied to clipboard!")).catch(()=>this._setOutput(`Share URL:
${l}`))}clearOutput(){this._setOutput("// Run code to see output here")}};export{r as PlaygroundController};
