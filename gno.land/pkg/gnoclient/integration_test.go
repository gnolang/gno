...
func Echo(str string) string {
	...
	require.NoError(t, err)
	assert.Equal(t, std.Coins{std.Coin{Denom: "ugnot", Amount: 176100}}, baseAcc.GetCoins())
...
func Hello(str string) string {
	...
	require.NoError(t, err)
	assert.Equal(t, "176100ugnot", baseAcc.GetCoins().String())
	...
	require.NoError(t, err)
	assert.Equal(t, std.Coins{std.Coin{Denom: "ugnot", Amount: 177200}}, baseAcc.GetCoins())
	...
	baseAcc, _, err = client.QueryAccount(caller.GetAddress())
	require.NoError(t, err)
	// 999999654670 = 10000000000000 - (GasFee 2100000 + Storage Deposit 176100 + Storage Deposit 177200 + Send 1000000)
	assert.Equal(t, std.Coins{std.Coin{Denom: "ugnot", Amount: 9999996546700}}, baseAcc.GetCoins())
