...
type mockVMKeeper struct {
	...
	queryFn                     func(sdk.Context, vm.EvalCfg) (string, error)
...
func (m *mockVMKeeper) QueryEval(ctx sdk.Context, cfg vm.EvalCfg) (res string, err error) {
	if m.queryFn != nil {
		return m.queryFn(ctx, cfg)
	}
