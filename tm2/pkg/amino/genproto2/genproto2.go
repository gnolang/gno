...
func (ctx *P3Context2) GenerateProtobuf3ForTypes(pkg string, rtz ...reflect.Type) (string, error) {
	...
	generatedTypes := make([]string, 0, len(rtz)) // type names for init() registration
	for _, rt := range rtz {
	...
	// Write imports goimports-style: stdlib first, then a blank line, then
	// third-party. Third-party paths contain a dot in the first path segment
	// (e.g. github.com/...). Matches what `goimports -w .` produces so that
	// `make generate` is a no-op after the generator runs.
	aliases := make([]string, 0, len(ctx.imports))
	for alias := range ctx.imports {
		aliases = append(aliases, alias)
	}
	sort.Strings(aliases)
	writeImport := func(alias string) {
		p := ctx.imports[alias]
		if alias == pathBase(p) {
	...
	for _, a := range aliases {
		if isThirdParty(ctx.imports[a]) {
			thirdParty = append(thirdParty, a)
		} else {
			stdlib = append(stdlib, a)
		}
	}
	sb.WriteString("import (\n")
	for _, a := range stdlib {
		writeImport(a)
	}
