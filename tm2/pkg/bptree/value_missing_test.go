...
func TestGetValue_MissingValueReturnsError(t *testing.T) {
	...
	// Commit so the value is persisted to the DB and cleared from the staging
	// buffer; GetValue must then resolve it from the DB (not pendingVals), so
	// deleting the DB row genuinely simulates corruption.
