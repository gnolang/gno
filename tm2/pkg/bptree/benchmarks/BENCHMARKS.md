...
Results"), which is the empirical basis for the mounted store and its gas
params. Still genuinely pending from the TODO below: flat-cost validation
(ReadCostFlat/WriteCostFlat), steady-state disk footprint (incl. the fast
index's value duplication), and index-ON latency/miss-mix at scale. The
33M data below remains valid history.
