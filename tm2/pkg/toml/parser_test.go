...
func assertSubTree(t *testing.T, path []string, tree *Tree, err error, ref map[string]any) {
	if err != nil {
	...
	for k, v := range ref {
		...
		case []*Tree:
			...
			for idx, item := range node {
				assertSubTree(t, nextPath, item, err, v.([]map[string]any)[idx])
			}
		case *Tree:
			t.Log("\tcomparing key", nextPath, "by subtree assestion")
			assertSubTree(t, nextPath, node, err, v.(map[string]any))
		default:
...
func assertTree(t *testing.T, tree *Tree, err error, ref map[string]any) {
...
func TestSimpleKV(t *testing.T) {
	tree, err := Load("a = 42")
	assertTree(t, tree, err, map[string]any{
		"a": int64(42),
	})
...
func TestHexIntegers(t *testing.T) {
	...
	assertTree(t, tree, err, map[string]any{"a": int64(3735928559)})
...
func TestOctIntegers(t *testing.T) {
	...
	assertTree(t, tree, err, map[string]any{"a": int64(342391)})

	tree, err = Load(`a = 0o755`)
	assertTree(t, tree, err, map[string]any{"a": int64(493)})
...
func TestBinIntegers(t *testing.T) {
	...
	assertTree(t, tree, err, map[string]any{"a": int64(214)})
...
func TestParseKeyGroupArraySpec(t *testing.T) {
	...
	assertTree(t, tree, err, map[string]any{
		"fruit": []map[string]any{
			{"name": "apple", "physical": map[string]any{"color": "red", "shape": "round"}},
			{"name": "banana"},
		},
	})
}
...
func TestToStringMapStringString(t *testing.T) {
	tree, err := TreeFromMap(map[string]any{"m": map[string]any{"v": "abc"}})
	if err != nil {
