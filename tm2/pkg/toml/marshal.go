...
var localDateType = reflect.TypeFor[LocalDate]()
var localTimeType = reflect.TypeFor[LocalTime]()
var localDateTimeType = reflect.TypeFor[LocalDateTime]()
...
func callCustomUnmarshaler(mval reflect.Value, tval any) error {
...
func (e *Encoder) valueToTree(mtype reflect.Type, mval reflect.Value) (*Tree, error) {
	...
	case reflect.Map:
		...
		for _, key := range keys {
			...
			if (mtype.Elem().Kind() == reflect.Pointer || mtype.Elem().Kind() == reflect.Interface) && mvalf.IsNil() {
				continue
			}
...
func (e *Encoder) valueToToml(mtype reflect.Type, mval reflect.Value) (any, error) {
	...
	default:
		...
		case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
			if mtype.Kind() == reflect.Int64 && mtype == reflect.TypeFor[time.Duration]() {
...
func (e *Encoder) wrapTomlValue(val any, parent *Tree) any {
	_, isTree := val.(*Tree)
	_, isTreeS := val.([]*Tree)
	if isTree || isTreeS {
		e.line++
...
// Convert toml value to marshal primitive slice, using marshal type
func (d *Decoder) valueFromOtherSliceI(mtype reflect.Type, tval any) (reflect.Value, error) {
...
func (d *Decoder) valueFromToml(mtype reflect.Type, tval any, mval1 *reflect.Value) (reflect.Value, error) {
	if mtype.Kind() == reflect.Pointer {
		return d.unwrapPointer(mtype, tval, mval1)
	}
	...
	case *Tree:
		...
		if mtype.Kind() == reflect.Interface {
			if mval1 == nil || mval1.IsNil() {
				return d.valueFromTree(reflect.TypeFor[map[string]any](), t, nil)
			} else {
	...
	case []*Tree:
		...
		if mtype.Kind() == reflect.Interface {
			if mval1 == nil || mval1.IsNil() {
				return d.valueFromTreeSlice(reflect.TypeFor[[]map[string]any](), t)
			} else {
				ival := mval1.Elem()
	...
	case []any:
		...
		if mtype.Kind() == reflect.Interface {
			if mval1 == nil || mval1.IsNil() {
				return d.valueFromOtherSlice(reflect.TypeFor[[]any](), t)
			} else {
				ival := mval1.Elem()
				return d.valueFromToml(mval1.Elem().Type(), t, &ival)
			}
	...
	default:
		...
		case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
			val := reflect.ValueOf(tval)
			if mtype.Kind() == reflect.Int64 && mtype == reflect.TypeFor[time.Duration]() && val.Kind() == reflect.String {
			...
			if reflect.Indirect(reflect.New(mtype)).OverflowInt(val.Convert(reflect.TypeFor[int64]()).Int()) {
		...
		case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64, reflect.Uintptr:
			...
			if val.Type().Kind() != reflect.Uint64 && val.Convert(reflect.TypeFor[int]()).Int() < 0 {
				return reflect.ValueOf(nil), fmt.Errorf("%v(%T) is negative so does not fit in %v", tval, tval, mtype.String())
			}
			if reflect.Indirect(reflect.New(mtype)).OverflowUint(val.Convert(reflect.TypeFor[uint64]()).Uint()) {
				return reflect.ValueOf(nil), fmt.Errorf("%v(%T) would overflow %v", tval, tval, mtype.String())
			}
		...
		case reflect.Float32, reflect.Float64:
			...
			if reflect.Indirect(reflect.New(mtype)).OverflowFloat(val.Convert(reflect.TypeFor[float64]()).Float()) {
				return reflect.ValueOf(nil), fmt.Errorf("%v(%T) would overflow %v", tval, tval, mtype.String())
			}
...
func (d *Decoder) unwrapPointer(mtype reflect.Type, tval any, mval1 *reflect.Value) (reflect.Value, error) {
...
func (d *Decoder) unmarshalText(tval any, mval reflect.Value) error {
	var buf bytes.Buffer
	fmt.Fprint(&buf, tval)
