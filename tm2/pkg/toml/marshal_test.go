...
func TestPointerCustomMarshalerSequence(t *testing.T) {
	...
	if !isCustomMarshalerSequence(reflect.TypeFor[*[]*customPointerMarshaler]()) {
		t.Errorf("error: should be a sequence of custom marshaler interfaces")
	}
	if !isCustomMarshalerSequence(reflect.TypeFor[*[2]*customPointerMarshaler]()) {
		t.Errorf("error: should be a sequence of custom marshaler interfaces")
	}
}
...
func TestPointerTextMarshalerSequence(t *testing.T) {
	...
	if !isTextMarshalerSequence(reflect.TypeFor[*[]*textPointerMarshaler]()) {
		t.Errorf("error: should be a sequence of text marshaler interfaces")
	}
	if !isTextMarshalerSequence(reflect.TypeFor[*[2]*textPointerMarshaler]()) {
		t.Errorf("error: should be a sequence of text marshaler interfaces")
	}
}
...
func TestMarshalInterface(t *testing.T) {
	...
	type OuterStruct struct {
		PrimitiveField        any
		ArrayField            any
		StructArrayField      any
		MapField              map[string]any
		StructField           any
		PointerField          any
		NilField              any
		InterfacePointerField *any
	}
...
`)
	...
	expected := OuterStruct{
		PrimitiveField: "Hello",
		ArrayField:     []any{int64(1), int64(2), int64(3)},
		StructField: map[string]any{
			"Field1": int64(123),
			"Field2": "Field2",
		},
		MapField: map[string]any{
			"MapField1": []any{int64(4), int64(5), int64(6)},
			"MapField2": map[string]any{
				"A": "A",
			},
			"MapField3": false,
		},
		...
		StructArrayField: []map[string]any{
			{
				"Name": "Allen",
				"Age":  int64(20),
			},
...
func TestUnmarshalSliceFail(t *testing.T) {
	tree, _ := TreeFromMap(map[string]any{
		"str_slice": []int{1, 2},
	})
...
func (d *docUnmarshalTOML) UnmarshalTOML(i any) error {
	if iMap, ok := i.(map[string]any); !ok {
		return fmt.Errorf("type assertion error: wants %T, have %T", map[string]any{}, i)
...
func (c *Custom) UnmarshalTOML(v any) error {
	c.v = "called"
	return nil
}
...
func (d *durationString) UnmarshalTOML(v any) error {
...
func (e *config437Error) UnmarshalTOML(v any) error {
