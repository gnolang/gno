...
func (f *failingWriter) Write(p []byte) (n int, err error) {
	count := len(p)
	toWrite := max(f.failAt-(count+f.written), 0)
	if toWrite > count {
		f.written += count
		f.buffer.Write(p)
		return count, nil
...
points = { x = 1, y = 2 }`)
	...
	assertTree(t, reparsedTree, err, map[string]any{
		"name": map[string]any{
			"first": "Tom",
			"last":  "Preston-Werner",
		},
...
func TestTreeWriteToTomlStringKeysOrders(t *testing.T) {
	for range 100 {
		...
		assertTree(t, toml, err, map[string]any{
			"foobar": true,
			"bar":    "baz",
			"foo":    1,
			"qux": map[string]any{
...
func testMaps(t *testing.T, actual, expected map[string]any) {
...
func TestTreeWriteToInvalidTreeSimpleValue(t *testing.T) {
	tree := Tree{values: map[string]any{"foo": int8(1)}}
	_, err := tree.ToTomlString()
	assertErrorString(t, "invalid value type at foo: int8", err)
}

func TestTreeWriteToInvalidTreeTomlValue(t *testing.T) {
	tree := Tree{values: map[string]any{"foo": &tomlValue{value: int8(1), comment: "", position: Position{}}}}
	_, err := tree.ToTomlString()
...
func TestTreeWriteToMapWithTablesInMultipleChunks(t *testing.T) {
	...
	expected := map[string]any{
		"menu": map[string]any{
			"main": []any{
				map[string]any{"a": "menu 1", "b": "menu 2"},
				map[string]any{"c": "menu 3", "d": "menu 4"},
			},
		},
	}
	treeMap := tree.ToMap()
