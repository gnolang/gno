...
func AddressToBech32(addr Address) string {
	bech32Addr, err := bech32.Encode(GetBech32AddrPrefix(), addr[:])
	if err != nil {
		panic(err)
	}
	return bech32Addr
...
func AddressFromBech32(bech32str string) (Address, error) {
	bz, err := GetFromBech32(bech32str, GetBech32AddrPrefix())
	if err != nil {
		return Address{}, err
	} else {
...
func PubKeyToBech32(pub PubKey) string {
	bech32PubKey, err := bech32.Encode(GetBech32PubKeyPrefix(), pub.Bytes())
	if err != nil {
		panic(err)
	}
	return bech32PubKey
...
func PubKeyFromBech32(bech32str string) (pubKey PubKey, err error) {
	bz, err := GetFromBech32(bech32str, GetBech32PubKeyPrefix())
	if err != nil {
