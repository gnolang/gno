...
func TestBlockRoundTripPreservesNilLastCommitEntries(t *testing.T) {
	...
	// Set Header.LastBlockID to match the commit's BlockID. ValidateBasic
	// (per the rule added in PR #5486 to harden non-genesis blocks) rejects
	// a non-genesis block with a zero LastBlockID but non-empty LastCommit;
	// the test's purpose is codec roundtrip fidelity, not that validation
	// rule, so populate the field as a real consensus path would.
