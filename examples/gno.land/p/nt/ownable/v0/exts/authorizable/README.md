...
func AddModerator(cur realm, addr address) error {
    return auth.AddToAuthList(addr, cur)
}
...
func (a *Authorizable) AddToAuthList(addr address, rlm realm) error
func (a *Authorizable) DeleteFromAuthList(addr address, rlm realm) error
...
- Every method takes the caller's own captured `cur` as `rlm` and asserts `rlm.IsCurrent()`, blocking the designation-forgery read where a non-crossing wrapper makes the realm walk return the wrong address. The membership checks have no other parameter for the realm to follow, so they keep a leading `_ int` placeholder: pass `0`.
