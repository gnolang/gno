...
- **T1 (Core Tier)**  
  - Highest tier; self-selecting membership with a *supermajority* vote from T1.
  - Membership can only be withdrawn by supermajority vote *with cause*.
- **T2**  
  - Selected by GovDAO with T3 abstaining, requiring a *simple majority* vote.
  - Membership can be withdrawn for any reason.
- **T3 (General Tier)**  
  - Lowest tier; *permissionless invitation* from T1 and T2 members.
  - Membership can be withdrawn for any reason.
...
- **T1 member**: 3 votes each.
- **T2 member**: 2 votes each.
- **T3 member**: 1 vote each.

### 3.2 Class Voting Power Caps
- **T2 class** is capped at *2/3* of the total T1 voting power.
- **T3 class** is capped at *1/3* of the total T1 voting power.

In practice, this often results in a voting-power ratio of:  
- **T1**: 1/2 of total votes  
- **T2**: 1/3 of total votes  
- **T3**: 1/6 of total votes  
...
3. **Membership Proposals.**  
   ...
   - Each proposal should include a Markdown resume/portfolio.
...
- **Invitation Points**:
  - Each **T1** member has 3 invitation points.
  - Each **T2** member has 2 invitation points.
  - Each **T3** member has 1 invitation point.
...
(You can include a section for tracking document changes or clarifications as the GovDAO evolves.)
