...
| **M24** ⚠️ | **NEW (ported from the NEWISSUES consensus-review log, R7): empty-key store semantics diverge from IAVL.** bptree rejects empty keys (`Set` → `ErrEmptyKey`, surfacing as a panic through the store wrapper) while IAVL accepts and stores them; the cache/prefix layers above reject only nil, so a non-nil empty key reaches the base store. Not a live bug (bptree unmounted; gno keys are prefixed/non-empty in practice), but a **promotion-gating compat decision**: swapping bptree in would panic on any code path or migrated state relying on IAVL's acceptance — resolve before promotion (accept empty keys for parity, or prove no empty key can exist in migrated state). | NEWISSUES R7 |
