...
| ~~M24~~ 🚫 | **WONTFIX (decided 2026-06-10): empty-key store semantics diverge from IAVL — bptree keeps the rejection.** bptree rejects empty keys (`Set` → `ErrEmptyKey`) while IAVL accepts and stores them. **Decision: not fixing for parity, because ICS23 cannot prove an empty key anyway** (`leaf op needs key` — IAVL hits the identical failure), so an empty-key entry would be unprovable state in either tree; rejecting at `Set` beats silently storing data that can never be proven. Residual (accepted): a code path or migrated state relying on IAVL's acceptance would panic under bptree — gno keys are prefixed/non-empty in practice; if migration ever surfaces one, that caller is the bug. | NEWISSUES R7 |
| ~~**M23**~~ ✅ | **FIXED (`bc8e50015`)** — the Importer now validates the stream structurally: leaf keys strictly ascend stream-wide (rejects unsorted/duplicates), leaf markers drain the buffer exactly, separators must sit in `max(left subtree) < sep <= min(right subtree)` (checkable exactly at the marker — post-order delivery means full subtree min/max is known), and inner heights must equal derived child height+1 (uniform children). With the caller's root-hash check, the only residual freedom is moving a separator WITHIN its window — provably unable to affect reads/iteration/prune; surfaces as a loud app-hash mismatch at the first divergently-routing write. Companion: Importer is now a lifecycle state machine (Close rolls back uncommitted staged values — the abort-leak; failed Commit poisons against the retry that committed a valueless tree under a MATCHING app hash; version/initialVersion restored on failure). Was: separator count/length checked but not ordering/consistency. | this review |
| ~~**M18**~~ ✅ | **FIXED (`bc8e50015`)** — `Import(version)` now requires `version > latestVersion` (a pruned version passed the old `VersionExists`-only check while its `{v, nonce}` key namespace was still shared into retained versions). Guard sits before the entry `Rollback` so a rejected Import has no side effects; gap versions beyond latest stay legal (fresh namespaces). Regression: `TestImport_RejectsNonFutureVersion` (incl. the pruned-version corruption hole). Original finding: **NEW (found by the L5 pass): `Import` to a PRUNED version number corrupts retained versions.** `Import(version)` only checks `!VersionExists(version)` (import.go:32) — a pruned version qualifies — and the import then reuses that version's node/value key namespaces `{v, nonce}`, overwriting nodes/values still **shared into retained versions**: empirically 99/100 wrong keys in a retained version from a fresh handle (silent; reads don't verify valueHash), plus `latestVersion` rewound below `firstVersion`. **Latent** — `Import` has no production caller (no state-sync). Fix when wiring state-sync: require `version > latestVersion` (or an empty version space) in `Import`. | this review |
...
- **Reachable leaks/robustness remaining: 0 of the original Tier B.** ✅ H11/H13 (`568bc03b6`); ✅ **M9** (`06de00540`) + ✅ **L3** (`1c5d72670`), both verified complete by the H1+L3+M9 adversarial pass; ✅ **L7** (`8208eb3c1`); ✅ **L1/L6/M8** (`1db08015f`); ✅ **L2** (`fc5451526`); ⚠️ **L5** downgraded (not-a-bug post-H2/H3); ⚠️ **M15** documented-contract (structural fix at state-sync). **NEW from the fuzzer-design + twin-fix reviews:** ✅ **M19** (twin node-record leak — the real leak half of C1) and ✅ **M20** (loaded-version prune brick + divergent-idempotent-replay corruption) fixed (`c15626f6e`); ✅ **M22** (prune decided existence via error-swallowing `VersionExists` at two destructive sites — found and fixed by the H1+L3+M9 pass); ✅ **M18** (version > latest guard) + ✅ **M23** (structural stream validation + importer state machine) fixed (`bc8e50015`); ⚠️ **M21** remains latent (pre-import value wall — fix via value reconciliation or import-into-fresh-DB at state-sync wiring). 🚫 **M24** empty-key IAVL-parity divergence — **WONTFIX by decision**: ICS23 can't prove an empty key in either tree, so bptree keeps rejecting at `Set` rather than storing unprovable state. **NEW hardening landed (from the newissues2 audit): record checksums (`159891004`, CRC-32C on every persisted record — corrupt metadata/values/roots now fail loud instead of serving wrong-but-hash-valid reads, wrong gas, or silent empty-tree adoption) and copy-on-return (`5de0e6265`, closes the cross-backend mutate-a-Get-result divergence and the Iterator.Key/Iterate/Export aliasing corruption).**
- **Latent concurrency (dormant under ABCI mutex): 0 of the original Tier C.** ✅ **H1** (`ac3d2756f`, verified complete by the H1+L3+M9 adversarial pass); ✅ **M11/M12** (`0de551f17`); ✅ **pendingVals** (`75c946820`, §VIII); ✅ **H2/H3** (`a07f4b3ce`, §IX); ✅ **M13** (`27c6ebbe5`, `GetNode` singleflight). Single-tree node reads, value resolution, prune-vs-reader, AND concurrent node loads are now safe against a writer. **Residual (addressed by contract — §X):** the pre-existing `MutableTree.version`/`lastSaved`/`root` read-vs-write field race is now documented as a single-goroutine contract + named `-race` guard (`446e4a6ad`, option a); an `RWMutex` on `MutableTree` (option b) is the only remaining promotion-time work. **NEW fail-loud hardening landed (from the newissues2 audit):** corrupt node records now ERROR instead of killing the process (`c92f74d54` — descents propagate; the store wrapper enforces iterator-error acknowledgment so unchecked failures stay loud rather than reading as silent truncation); a session poison flag makes every post-mutation failure refuse SaveVersion until Rollback (`4a0e8b8df` — closes the silent-commit-after-error class incl. the failed-save retry that bricked a version; LoadVersion now reads before wiping so failed loads keep the session intact); IterateRange + Query /subspace propagate iteration errors (`a8a83424d`).
