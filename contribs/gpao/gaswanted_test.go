...
// the caller's stand-in rather than to zero or to no bound: zero would make
// every approval fail instantly, and no bound would let one absurd estimate ask
// for unbounded gas.
func TestBlockMaxGasFrom(t *testing.T) {
	...
	// The caller's stand-in, distinct from any real ceiling so a fallback
	// cannot be mistaken for a value the response carried.
	const fallback = int64(9_999)

	t.Run("the chain's own value is used", func(t *testing.T) {
		assert.Equal(t, int64(500_000), blockMaxGasFrom(withMaxGas(500_000), fallback))
	})
	...
	t.Run("no bound falls back rather than being taken literally", func(t *testing.T) {
		...
		assert.Equal(t, fallback, blockMaxGasFrom(withMaxGas(-1), fallback))
		assert.Equal(t, fallback, blockMaxGasFrom(withMaxGas(0), fallback))
	})

	t.Run("a malformed response falls back", func(t *testing.T) {
		assert.Equal(t, fallback, blockMaxGasFrom(nil, fallback))
		assert.Equal(t, fallback,
			blockMaxGasFrom(&ctypes.ResultConsensusParams{}, fallback),
