...
func (m model) fetchRenderView() (view []byte, err error) {
	...
	res, err := makeRender(noopLogger, rlmpath)
	if err != nil {
...
func (m model) fetchFuncsList() (view []list.Item, err error) {
	...
	funcs, err := makeFuncs(noopLogger, rlmpath)
	if err != nil {
		return nil, fmt.Errorf("unable to fetch Render: %w", err)
	}
	...
	case tea.KeyMsg:
		...
		case "enter":
			if m.commandInput.Focused() && !m.messageDisplay {
				...
				path := m.urlInput.Value()
				rlmpath := gnoPrefix + path
				res, err := makeCall(noopLogger, makeCallCfg{
					kb:           m.kb,
					pass:         m.pass,
					nameOrBech32: m.name,
					eval:         m.commandInput.Value(),
					rlmpath:      rlmpath,
				})
...
func main() {
	...
	p := tea.NewProgram(
		model{
			kb:   kb,
			name: name, pass: string(password),
