...
This installs `gno`, `gnokey`, `gnodev`, `gnobro`, and `gnoweb`. Without `--dir`
the installer defaults to `$HOME/.gno/bin`, which works but won't match
source-built binaries — keeping a single directory avoids stale-binary
shadowing on your `PATH`.
...
| `GNO_INSTALL_DIR` | `--dir` | Installation directory (script default: `$HOME/.gno/bin`; recommended: `$(go env GOPATH)/bin`) |
...
Official Docker images are available at [`ghcr.io/gnolang/gno`](https://ghcr.io/gnolang/gno).
You can use them to run individual tools without installing from source:
...
**macOS Gatekeeper blocks the binary** — "cannot be opened, developer cannot be
verified". Clear the quarantine attribute:

```sh
xattr -d com.apple.quarantine "$(go env GOPATH)/bin/"*
```
