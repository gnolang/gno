...
Install the Gno toolchain: `gnokey` (key & transaction CLI),
...
| `GITHUB_TOKEN` | (none) | Authenticates GitHub API requests; raises the 60 requests/hour anonymous rate limit |
...
# Install core tools (gnokey, gno, gnodev)
make install
```
...
make install.gnobro              # Only gnobro
make -C gno.land install.gnoweb  # Only gnoweb
```
...
- [Using the `gnokey` wallet](../users/using-gnokey.md): manage keys and send transactions
