	...
	for index := 0; index < count; index++ {
		for _, messageType := range messageTypes {
			...
			case MessageType_PROPOSAL:
				message.Payload = &Message_ProposalMessage{
			...
			case MessageType_PREVOTE:
				message.Payload = &Message_PrevoteMessage{
					PrevoteMessage: &PrevoteMessage{
						From: []byte(strconv.Itoa(index)),
						View: view,
					},
				}
			case MessageType_PRECOMMIT:
				message.Payload = &Message_PrecommitMessage{
...
func TestCollector_AddMessage(t *testing.T) {
	...
	t.Run("empty message queue", func(t *testing.T) {
		...
		c := NewCollector[ProposalMessage]()
	...
	t.Run("valid PROPOSAL messages fetched", func(t *testing.T) {
		...
		expectedMessages := make([]*ProposalMessage, 0, count)

		for _, message := range generatedMessages {
			proposal, ok := message.Payload.(*Message_ProposalMessage)
			require.True(t, ok)
	...
	t.Run("valid PREVOTE messages fetched", func(t *testing.T) {
		...
		expectedMessages := make([]*PrevoteMessage, 0, count)

		for _, message := range generatedMessages {
			prevote, ok := message.Payload.(*Message_PrevoteMessage)
			require.True(t, ok)
	...
	t.Run("valid PRECOMMIT messages fetched", func(t *testing.T) {
		...
		// Create the collector
		c := NewCollector[PrecommitMessage]()
		...
		expectedMessages := make([]*PrecommitMessage, 0, count)

		for _, message := range generatedMessages {
			precommit, ok := message.Payload.(*Message_PrecommitMessage)
			require.True(t, ok)
...
func TestCollector_AddDuplicateMessages(t *testing.T) {
	...
	var (
		...
		commonType   = MessageType_PREVOTE
		view         = &View{
			Height: 1,
			Round:  1,
		}
